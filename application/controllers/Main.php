...
class Main extends CI_Controller
{
	...
	//     // Ambil semua data diklat, urut dari terbaru
	//     $data['daftar'] = $this->db->order_by('id', 'DESC')->get('diklat')->result();

	//     // Debug: cek apakah data muncul semua
	// //     echo '<pre>';
	// //     print_r($data['daftar']);
	// //     echo '</pre>';
	// //     echo 'Jumlah data: ' . count($data['daftar']);
	// //     exit; // hentikan eksekusi agar tidak lanjut ke view saat debugging


	//     $this->load->view('frontview/header', $data);
	//     $this->load->view('frontview/navbar', $data);
	//     $this->load->view('frontview/page/diklat/diklat', $data);
	//     $this->load->view('frontview/footer', $data);
	// }

	// Fungsi yang sudah ada (pastikan ada)
	public function diklat()
	{
		$data['title_bar'] = "Pendidikan dan Pelatihan";
		$data['header_page'] = "";
	...
	// TAMBAHKAN FUNGSI INI DI CONTROLLER MAIN
	public function diklat_detail($id)
	{
		...
		$diklat = $this->db->where('id', $id)->get('diklat')->row();

		// Jika data tidak ditemukan, redirect ke halaman diklat
		if (!$diklat) {
			redirect('diklat');
			return;
		}

		$data['title_bar'] = $diklat->nama;
		$data['header_page'] = "";
		$data['diklat'] = $diklat;

		$this->load->view('frontview/header', $data);
		$this->load->view('frontview/navbar', $data);
		$this->load->view('frontview/page/diklat/diklat_detail', $data);
		$this->load->view('frontview/footer', $data);
	}
...
// 	}
    ...
    public function dokter()
    {
        ...
        // --- LOAD POSTER JADWAL DOKTER ---
        $query2 = "SELECT * FROM jadwaldokter ORDER BY id DESC";
        $data['jadwaldokter'] = $this->db->query($query2)->result();
    
        // --- LOAD DAFTAR DOKTER ---
        $query3 = "SELECT * FROM dokter ORDER BY id DESC";
        $data['daftar'] = $this->db->query($query3)->result();
